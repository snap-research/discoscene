# python3.7
"""Defines loss functions for StyleGAN3 training."""
...
__all__ = ['StyleGAN3Loss']


class StyleGAN3Loss(BaseLoss):
    """Contains the class to compute losses for training StyleGAN3.
    ...
    NOTE: This function differs from StyleGAN2 training from the following
    perspectives:

    (1) Blur the image before feeding the image to the discriminator. Please
        refer to `blur_init_sigma` and `blur_fade_img`.
    (2) Optionally update statics in the generator. Please refer to
        `update_ema`.
    (3) Turn of perceptual path length regularization by default. Please refer
        to `pl_weight` and `pl_interval`.
    """
    ...
    def __init__(self, runner, d_loss_kwargs=None, g_loss_kwargs=None):
        ...
        if runner.enable_amp:
            raise NotImplementedError('StyleGAN3 loss does not support '
        ...
        # Blur the images shown to the discriminator.
        self.blur_init_sigma = self.d_loss_kwargs.get('blur_init_sigma', 0.0)
        # Fade out the image blurry at the beginning.
        blur_fade_img = self.d_loss_kwargs.get('blur_fade_img', 0.0)
        self.blur_fade_img = blur_fade_img * runner.minibatch / 32
        ...
        self.pl_weight = self.g_loss_kwargs.get('pl_weight', 0.0)
        ...
        self.pl_interval = self.g_loss_kwargs.get('pl_interval', 0)
        ...
        runner.logger.info('image blurry settings (blurring images shown to D)',
                           indent_level=1)
        runner.logger.info(f'blur_init_sigma: {self.blur_init_sigma}',
                           indent_level=2)
        runner.logger.info(f'blur_fade_img: {self.blur_fade_img}',
                           indent_level=2)
    ...
    def run_G(runner,
        ...
        magnitude_moving_decay = 0.5 ** (runner.minibatch / 20_000)
        with ddp_sync(G, sync=sync):
            return G(latents, labels,
                     magnitude_moving_decay=magnitude_moving_decay,
                     update_ema=update_ema,
                     **G_kwargs)

    def run_D(self, runner, images, labels, sync=True):
        """Forwards discriminator."""
        # Blur the images if needed.
        blur_size = 0
        if self.blur_fade_img > 0 and self.blur_init_sigma > 0:
            fade_factor = 1 - runner.seen_img / self.blur_fade_img
            blur_sigma = max(fade_factor, 0) * self.blur_init_sigma
            blur_size = np.floor(blur_sigma * 3)
        if blur_size > 0:
            f = torch.arange(-blur_size, blur_size + 1, device=images.device)
            f = f.div(blur_sigma).square().neg().exp2()
            impl = runner.model_kwargs_train['discriminator'].get(
                'impl', 'cuda')
            images = upfirdn2d.filter2d(images, f / f.sum(), impl=impl)
    ...
    def d_fake_loss(self, runner, _data, sync=True):
        ...
        fake_results = self.run_G(runner, update_ema=True, sync=False)
        fake_scores = self.run_D(runner,
                                 images=fake_results['image'],
                                 labels=fake_results['label'],
                                 sync=sync)['score']
