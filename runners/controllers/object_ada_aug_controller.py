...
class ObjectAdaAugController(BaseController):
    ...
    def setup(self, runner):
        ...
        if not isinstance(runner.object_augment, AdaAug):
        ...
        runner.object_augment.p = torch.as_tensor(aug_prob).relu()
        runner.object_augment.prob_tracker = SingleStats('Object Aug Prob Tracker',
                                                  log_format=None,
                                                  log_strategy='AVERAGE',
                                                  requires_sync=True)
        runner.running_stats.add('Misc/Object Aug Prob',
                                 log_name='object_aug_prob',
                                 log_format='.3f',
    ...
    def execute_after_iteration(self, runner):
        if self._strategy == 'fixed':
            ...
            criterion = runner.object_augment.prob_tracker.summarize()
            adjust = np.sign(criterion - self._target_p) * slope
            aug_prob = runner.object_augment.p + adjust

        runner.object_augment.p = torch.as_tensor(aug_prob).relu()
        runner.running_stats.update({'Misc/Object Aug Prob': runner.object_augment.p})
