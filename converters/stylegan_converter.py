# python3.7
"""Converts StyleGAN model weights.
...
class StyleGANConverter(BaseConverter):
    """Defines the converter for converting StyleGAN model."""

    def __init__(self, verbose_log=False):
        if tf is None:
            raise SystemExit('Converting StyleGAN relies on the module '
    ...
    def parse_model_config(self):
        ...
        self.dst_kwargs['discriminator'] = dict(
            model_type='StyleGANDiscriminator',
            resolution=resolution,
            label_dim=label_dim,
            image_channels=image_channels)
    ...
    def convert_generator(src_model, dst_model, log_fn=None):
        ...
        for dst_name, src_name in dst_to_src_mapping.items():
            ...
            if 'weight' in src_name:
                ...
                elif 'Dense' in src_name:
                    var = var.permute(1, 0)
                elif 'StyleMod' in src_name:
                    var = var.permute(1, 0)
                elif 'LabelConcat' in src_name:
                    pass
                elif 'Noise' in src_name:
        ...
        for i in range(num):
            ...
            with torch.no_grad():
                dst_image = G_dst(latent_pth, label_pth,
                                  ...
                                  trunc_layers=TRUNC_LAYERS,
                                  noise_mode='const')['image']
                dst_score = D_dst(dst_image, label_pth)['score']
            # Compare.
