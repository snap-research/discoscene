...
try:
    import models.clib._ext as _ext
except ImportError:
    pass
    # raise ImportError(
...
class AABBRayIntersect(Function):
    ...
    def forward(ctx, voxelsize, n_max, points, ray_start, ray_dir):
        ...
        # G = min(32, int(2 * 10 ** 9 / points.numel()))   # HACK: avoid out-of-memory
        ...
        # ray_start = ray_start.reshape(G*S, K, 3)
        # ray_dir = ray_dir.reshape(G*S, K, 3)
        # points = points.repeat_interleave(G, dim=0).contiguous()

        # assert G%S == 0, 'bs must be divided by 2048'
        K = int(np.ceil(S*N/G))
        H = int(K*G/S)
        if H > N:
            ray_start = torch.cat([ray_start, ray_start[:, :H-N]], 1)
            ray_dir = torch.cat([ray_dir, ray_dir[:, :H-N]], 1)
        ray_start = ray_start.reshape(G, K, 3).contiguous()
        ray_dir = ray_dir.reshape(G, K, 3).contiguous()
        # points = points.expand(S * G, *points.size()[1:]).contiguous()
        points = points.repeat_interleave(G//S, dim=0).contiguous()
